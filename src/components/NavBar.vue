    ...
    <nav class="bg-white border-b-gray-200 sticky w-full top-0 z-50 shadow-md shadow-slate-500">
        <div class="max-w-7xl mx-auto px-4 py-2 sm:px-6 lg:px-8 md:flex md:justify-between">
            ...
            <div class="hidden md:flex md:items-center md:justify-between">
                <router-link :to="{ name: 'policy', params: { policy: 'about' }}" class="px-4 py-2 hover:text-rose-600 hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'">About</router-link>

                <template v-if="authStore.isAuthenticated">
                    <router-link :to="{ name: 'account' }" class="px-4 py-2 hover:text-rose-600 hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'"><i class="pi pi-user text-sm mr-2"></i>Account</router-link>
        ...
        <div class="max-w-7xl mx-auto px-4 py-2 sm:px-6 lg:px-8 md:flex md:justify-between text-center">
            ...
            <div class="md:flex md:items-center md:justify-between" :class="openMenu ? 'block' : 'hidden'">
                <router-link :to="{ name: 'home' }" class="px-4 py-2 hover:text-rose-600 block md:inline hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'">Home</router-link>
                <router-link :to="{ name: 'policy', params: { policy: 'about' }}" class="px-4 py-2 block md:hidden hover:text-rose-600 hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'">About</router-link>

                <template v-if="authStore.isAuthenticated">
                    <router-link :to="{ name: 'account' }" class="px-4 py-2 block md:hidden hover:text-rose-600 hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'"><i class="pi pi-user text-sm mr-2"></i>Account</router-link>
                    <router-link :to="{ name: 'wishlist' }" class="px-4 py-2 hover:text-rose-600 block md:inline hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'"><i class="pi pi-heart text-sm mr-2"></i>Wish</router-link>
                    <router-link :to="{ name: 'cart' }" class="px-4 py-2 hover:text-rose-600 block md:inline hover:bg-slate-50 transition-all" :activeClass="'text-rose-600 bg-slate-50'"><i class="pi pi-shopping-cart text-sm mr-2"></i>Cart</router-link>
                </template>
                ...
                <div>
                    ...
                    <div v-show="openCategories" class="md:mt-10 md:absolute md:top-100 md:right-12 xl:right-24 2xl:right-auto max-h-[50vh] overflow-auto w-full md:min-w-[150px] md:max-w-[300px] rounded border border-gray-50 bg-white">
                        <router-link :to="{ name: 'listing', params: { type: 'category', identifier: category.id }}" v-for="category in categoryStore.categories" :key="category.id" class="block px-4 py-2 hover:text-rose-600 hover:bg-slate-50 border-b border-b-gray-200 hover:border-b-rose-500 transition-all" :activeClass="'text-rose-600 bg-slate-50'">{{ category.name }}</router-link>
                    </div>
                </div>
